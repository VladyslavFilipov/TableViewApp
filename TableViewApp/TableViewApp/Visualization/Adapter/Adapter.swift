...
class Adapter: AdapterProtocol {
    ...
    func setUpControllers(by manager: ControlManagerProtocol, in view: UIStackView, and otherView: UIStackView?) {
        ...
        for item in array {
            ...
            case .textField(let placeholder, let keyboard, let action):
                otherView?.addArrangedSubview(CustomTextField(placeholder: placeholder, keyboard: keyboard, action: action))
